...
### Preparing data for the web portal

The CSV remains the master dataset. After it is updated, generate the compact,
validated web dataset with:

```powershell
python tools/build_web_data.py
```

The default output is `portal/public/data/magnum-history.json`. The formatter:

- validates four-digit numbers, dates, draw IDs, and prize types;
- removes exact duplicate result records;
- sorts every number's results chronologically;
- omits meanings and translated prize descriptions used only by the source CSV;
- records source counts, date coverage, and a SHA-256 checksum; and
- replaces the output atomically only after a successful build.

Validate the source and preview output statistics without writing a file:

```powershell
python tools/build_web_data.py --check
```

The generated JSON is committed for deployment, while the CSV, notebook,
analysis scripts, and graph images remain outside the Vercel `portal/` root.
